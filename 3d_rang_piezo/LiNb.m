...
d11 = zeros(length(theta),length(phi));
...
for i=1:length(theta)
  for j=1:length(phi)
    ...
    %d11(i,j)=d_tensor_new(3,3,3);

    % 1 оптимизация
    % d_tensor_new = d_tensor_d11_rotate(d_tensor, rot_matrix);
    % d11(i,j) = d_tensor_new(3,3,3);
...
[X, Y, Z] = sph2cart(THETA, -(PHI-pi/2), abs(d11));
...
figure
surf(rad2deg(THETA),rad2deg(PHI),d11);
[X, Y, Z] = sph2cart(THETA, -(PHI-pi/2), abs(d11));
daspect([1 1 1]);
